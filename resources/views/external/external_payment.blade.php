                    ...
                    <div id="checkout_wizard" class="checkout accordion left-chck145">
                        <div class="checkout-step">
                            <div class="checkout-card" id="headingPayment">
                                ...
                                <p><small>Transaction Ref.: {{ strtoupper($order->transaction_id) }}</small></p>
                            </div>
                            <div class="checkout-step-body">
                                <div class="payment_method-checkout">
                                    <form action="{{ route('external_payment.store', $order->transaction_id) }}" method="POST">
                    ...
                    <div class="pdpt-bg mt-0">
                        ...
                        <div class="right-cart-dt-body">
                            @foreach($order->items as $orderItems)
                                <div class="cart-item border_radius">
                                    <div class="cart-product-img">
                                        <img src="{{ asset('assets/common/ajax-loader-canvas.gif') }}"  data-src="{{ $orderItems->product->image ? asset($orderItems->product->image) : asset('assets/common/image_placeholder.png')}}" alt="{{ $orderItems->product->name }}" class="lazy">
                                        @if($orderItems->product->hasDiscount)
                                            <div class="offer-badge">{{ $orderItems->product->discount }}% OFF</div>
                                        @endif
                                    </div>
                                    <div class="cart-text">
                                        <h4 style="width: 80%">{{ $orderItems->product->name }}</h4>
                                        <div class="qty-group">
                                        ...
                                        <div class="cart-mutators">
                                            @foreach($orderItems->mutators as $orderItemsMutator)
                                                <span class="badge badge-dark pl-2 pr-2"><small style="font-size: 11px">{{ $orderItemsMutator->mutator->group->name }}: {{ $orderItemsMutator->mutator->name }}</small></span>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if($order->custom_product_request && is_array($order->custom_product_request) && count($order->custom_product_request) > 0)
                                @foreach($order->custom_product_request as $orderCustomItem)
                                    <div class="cart-item">
                                        <div class="cart-text">
                                            <h4 style="width: 80%">{{ $orderCustomItem }}</h4>
                                            <div class="qty-group">
                                                <div class="cart-item-price"><small style="font-size: 11px"></small></div>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
