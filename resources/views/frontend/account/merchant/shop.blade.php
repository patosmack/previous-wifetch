    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            let table = $('#list-datatable').DataTable({
                ...
                ajax:{
                    type: "POST",
                    url: "{{ route('account.merchant.shop.json', ['id' => $merchant->id, 'private_category_id' => $private_category]) }}",
            ...
            function sendRequest(element, target, ref, val, message) {
                ...
                $.ajax({
                    ...
                    url:   '{{ route('account.merchant.shop.easy-edit') }}',
                    type:  'POST',
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    beforeSend: function () {
...
@section('content')
    ...
    <form action="{{ route('account.merchant.shop', $merchant->id) }}" method="GET" id="private_category_form" class="d-none  float-left">
